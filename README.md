...
브릿지빈(Bridgebean) B2B 원두 공급사 포트폴리오 웹사이트 코드입니다.
...
4. 바꾸고 싶은 원두의 `"price_per_kg": 32000` 부분에서 숫자만 바꿉니다.
   - 단위는 **원(KRW)** 입니다. 콤마(`,`) 없이 숫자만 입력. 예: `35000`
...
2. 마지막 원두 (`}`로 끝나는 블록) 다음에 **콤마 `,`** 를 찍고, 아래 양식을 붙여넣기:
...
,
  {
    ...
    "roast_level": "라이트",
    "notes": ["블랙커런트", "토마토", "자몽"],
    "description": "이곳에 원두 소개 글을 자유롭게 적습니다.",
    "recommended_brew": "푸어오버",
    "best_for": "시그니처 핸드드립",
    "price_per_kg": 38000,
    "min_order_kg": 5,
...
| 항목 | 의미 | 예시 |
|---|---|---|
| `id` | 주소창에 들어가는 영문 식별자. 영어 소문자·숫자·하이픈만. **다른 원두와 중복되면 안 됩니다.** | `kenya-aa-nyeri` |
| `name` | 사이트에 보이는 한글 이름 | `케냐 AA 니에리` |
| `name_en` | 작은 영문 이름 (선택) | `Kenya AA Nyeri` |
| `category` | `"single-origin"` 또는 `"blend"` 중 하나만 | `single-origin` |
| `origin` | 산지 (국가/지역) | `케냐 니에리` |
| `farm` | 농장·스테이션·협동조합 | `OO 협동조합` |
| `altitude` | 재배 고도 | `1,700m` |
| `process` | 가공 방식 | `워시드`, `내추럴` 등 |
| `variety` | 품종 | `SL28` |
| `roast_level` | 로스팅 단계 | `라이트` / `미디엄` / `다크` |
| `notes` | 컵노트. 대괄호 `[]` 안에 큰따옴표로 넣고 콤마로 구분 | `["블랙커런트", "토마토"]` |
| `description` | 원두 소개 글 | 자유 작성 |
| `recommended_brew` | 추천 추출법 | `푸어오버` |
| `best_for` | 잘 어울리는 메뉴 | `시그니처 핸드드립` |
| `price_per_kg` | 1kg 도매가. 숫자만. | `38000` |
| `min_order_kg` | 최소 주문 kg | `5` |
| `image` | 원두 사진 경로. `/images/` 로 시작. | `/images/kenya-aa-nyeri.jpg` |
...
- 슬로건, 회사 설명, 3가지 강점, 로스터 철학을 직접 수정하시면 됩니다.
...
5. `data/coffees.json` 에서 해당 원두의 `image` 값을 `/images/올린파일이름.jpg` 로 바꿔주세요.
...
### 2-6. 공급 프로세스·최소 주문량·납품 정책을 바꿀 때
...
| 파일 | 어디에 보이나요? |
|---|---|
| `data/company.json` | 회사명, 슬로건, 회사 소개, 강점 3가지, 로스터 철학, 운영 원칙 |
| `data/coffees.json` | 원두 라인업 (원두 목록 페이지 + 각 원두 상세 페이지) |
| `data/services.json` | 공급 서비스 페이지 (프로세스 / 납품 / 최소 주문 / 추가 서비스) |
...
**Q. 도매가가 바뀌었어요. 한 원두만 빠르게 바꾸려면?**
A. `data/coffees.json` 열기 → 브라우저 검색(Ctrl/Cmd + F)으로 원두 이름 찾기 →
...
- 배포: Vercel — `main` 브랜치에 푸시되면 자동 배포
